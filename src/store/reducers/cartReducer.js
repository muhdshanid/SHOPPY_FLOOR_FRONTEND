...
const calculateTotal = data => {
    let total = 0
    for (let i = 0; i < data.length ; i++){
        total = total + discount(data[i].price,data[i].discount) * data[i].quantity
    }
    return total
}
const cartReducer = createSlice({
    ...
    reducers:{
       addToCart : (state,{payload}) => {
        ...
        state.total = state.total + discount(payload.price , payload.discount) * payload.quantity
       },
       incQuantity:(state,{payload}) => {
        ...
        if(find){
            find.quantity += 1
            state.items += 1
            state.total = Number(state.total) + Number(discount(find.price,find.discount))
